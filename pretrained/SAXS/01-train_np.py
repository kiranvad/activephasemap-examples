...
# (1, 100, 1)
x_target = torch.log10(torch.logspace(dataset.xrange[0], 
                                      dataset.xrange[1], 
                                      100)
...
for epoch in range(num_epochs+1):
    ...
    if (epoch)%plot_epochs_freq==0:
        ...
        torch.save(neural_process.state_dict(), checkpoint_path)
        with torch.no_grad():
            fig, ax = plt.subplots()
...
with torch.no_grad():
    ...
    if z_dim==2:
        plot_zgrid_curves([-1.96,1.96], x_target, neuralprocess)
