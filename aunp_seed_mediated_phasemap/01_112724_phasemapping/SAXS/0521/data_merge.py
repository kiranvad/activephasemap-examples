...
def combine_all_numpy(*args):
    ...
    if len(args) == 3:
        ESAXS = read_DAT_file(args[0])
        SAXS =  read_DAT_file(args[1])
        MAXS =  read_DAT_file(args[2])   

        ESAXS_ident = np.array([1]*len(ESAXS)).reshape(-1,1)
        SAXS_ident = np.array([2]*len(SAXS)).reshape(-1,1)
        MAXS_ident = np.array([3]*len(MAXS)).reshape(-1,1)
        ESAXS = np.hstack((ESAXS[:,0].reshape(-1,1), ESAXS[:,1].reshape(-1,1), ESAXS[:,2].reshape(-1,1), ESAXS_ident))
        SAXS = np.hstack((SAXS[:,0].reshape(-1,1), SAXS[:,1].reshape(-1,1), SAXS[:,2].reshape(-1,1), SAXS_ident))
        MAXS = np.hstack((MAXS[:,0].reshape(-1,1), MAXS[:,1].reshape(-1,1), MAXS[:,2].reshape(-1,1), MAXS_ident))

        main_array = np.vstack((ESAXS, SAXS, MAXS))

    elif len(args) == 2:
        ESAXS = np.load(args[0])
        SAXS =  np.load(args[1])
        ESAXS_ident = np.array([1]*len(ESAXS)).reshape(-1,1)
        SAXS_ident = np.array([2]*len(SAXS)).reshape(-1,1)
        ESAXS = np.hstack((ESAXS[:,0].reshape(-1,1), ESAXS[:,1].reshape(-1,1), ESAXS[:,2].reshape(-1,1), ESAXS_ident))
...
def perform_merge(**kwargs):
    ...
    axs[0].scatter(ESAXS[:,0], ESAXS[:,1], color = 'red', label="Data 1")
    axs[0].scatter(SAXS[:,0], SAXS[:,1], color = 'orange', label = "Data 2")
    if len(kwargs.keys()) == 6:
        axs[0].scatter(MAXS[:,0], MAXS[:,1], color = 'grey')
    axs[0].legend()
...
def perform_merge_numpy(**kwargs):
    '''This function merges 3 sets of data together based on two values specified in cut_locations, and plots the merged 
       and unmerged data.
       The keyword arguments for kwargs are: Data_1, Data_2, Data_3 (optional), cut_locations, idir, save.'''
    if len(kwargs.keys()) == 6:
        ...
        MAXS = kwargs['Data_3']
        idir = kwargs['idir']
        arr = combine_all_numpy(ESAXS, SAXS, MAXS)
        MAXS = np.load(MAXS)
    elif len(kwargs.keys()) == 5:
        ESAXS = kwargs['Data_1']
        SAXS = kwargs['Data_2']
        arr = combine_all_numpy(ESAXS, SAXS)
        idir = kwargs['idir']
    cut_locations = kwargs['cut_locations']
    arr, _ = cut(arr, cut_locations)
    ESAXS = np.load(ESAXS)
    SAXS = np.load(SAXS)
    ESAXS = np.hstack((ESAXS, ESAXS[:,1].reshape(-1,1), ESAXS[:,2].reshape(-1,1)))
    SAXS = np.hstack((SAXS, SAXS[:,1].reshape(-1,1), SAXS[:,2].reshape(-1,1)))
    ...
    #plt.scatter(10**(arr[:,0]), 10**(arr[:,1]))
    ax[0].scatter(ESAXS[:,0], ESAXS[:,1], color = 'red')
    ax[0].scatter(SAXS[:,0], SAXS[:,1], color = 'orange')
    ax[0].axvline(x = SAXS[cut_locations[0],0], color="k")
    if len(kwargs.keys()) == 6:
        ax[0].scatter(MAXS[:,0], MAXS[:,1], color = 'grey')
        ax[0].axvline(x = MAXS[cut_locations[1],0], color="k")
    ax[0].legend()
