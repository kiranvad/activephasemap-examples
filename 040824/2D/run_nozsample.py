...
def featurize_spectra(np_model, comps_all, spectra_all):
    ...
    train_x, train_y = [], []
    # We approximate the z value of any given curve by passing it through
    # NP model to obtain a mean and std of z
    with torch.no_grad():
        train_x = torch.from_numpy(comps_all)
        train_y, _ = np_model.xy_to_mu_sigma(t.unsqueeze(2), spectra.unsqueeze(2)) 
